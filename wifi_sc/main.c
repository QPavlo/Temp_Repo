...
static int getWifiInfo_callback(struct nl_msg *msg, void *arg) {
    ...
    if (tb[NL80211_ATTR_SSID]) {
        char *ssid_buf;
//        memcpy(ssid_buf, tb[NL80211_ATTR_SSID], sizeof(ssid_buf));

        ssid_buf = nla_get_string(tb[NL80211_ATTR_SSID]);

        printf("SSID: %s\n", ssid_buf);
    }

    if (tb[NL80211_ATTR_MAC]) {
        uint8_t *bssid = nla_data(tb[NL80211_ATTR_MAC]);
        ((Wifi *) arg)->bssid = bssid;
    }
...
int main(int argc, char **argv) {
    ...
    do {
        getWifiStatus(&nl, &w);
        printf("Interface: %s | bssid: %02x:%02x:%02x:%02x:%02x:%02x | signal: %d dB | txrate: %.1f MBit/s\n",
               w.ifname,
               w.bssid[0], w.bssid[1], w.bssid[2], w.bssid[3], w.bssid[4], w.bssid[5],
               w.signal, (float) w.txrate / 10);
        sleep(10);
    } while (keepRunning);
